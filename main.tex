...
Biodiversity data aggregators, such as DataONE, GBIF, and iDigBio, rely on data providers such as data curators and institutional repositories to maintain active dataset URLs, and aggregate the data found at those URLs for distribution in response to user queries. From here on, we use the term "data network" to refer to a collection of URLs that are discoverable through some central URL registry, and the term "provider network" to refer to the subset of URLs in a biodiversity aggregator's data network that provide biodiversity datasets to the aggregator. These provider network URLs are often used in publications as references for biodiversity datasets.
